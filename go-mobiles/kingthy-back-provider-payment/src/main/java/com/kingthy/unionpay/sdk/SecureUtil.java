...
public class SecureUtil
{
    ...
    public static String sm3X16Str(String data, String encoding)
    {
        ...
        sm3StrBuff = getString(bytes, sm3StrBuff);
        return sm3StrBuff.toString();
    }
    
    private static StringBuilder getString(byte[] bytes, StringBuilder sm3StrBuff)
    {
        if (null != bytes)
        {
    ...
    public static byte[] sha1X16(String data, String encoding)
    {
        ...
        sha1StrBuff = getString(bytes, sha1StrBuff);
        try
        {
    ...
    public static String sha256X16Str(String data, String encoding)
    {
		...
		sha256StrBuff = getString(bytes,sha256StrBuff);
...
//            操作数值2
            int operaValue = 2;
            if (tPinLen % operaValue == 0)
            {
                for (int i = 0; i < tPinLen;)
                {
                    String a = aPin.substring(i, i + operaValue);
                    tByte[tTemp] = (byte)Integer.parseInt(a, 16);
                    if (i == (tPinLen - operaValue))
                    {
                        if (tTemp < 7)
                        {
                            for (int x = (tTemp + 1); x < value; x++)
                            {
                                tByte[x] = (byte)0xff;
                            }
                        }
            ...
            else
            {
                ...
                {
                    String a;
                    a = aPin.substring(i, i + operaValue);
                    tByte[tTemp] = (byte)Integer.parseInt(a, 16);
                    if (i == (tPinLen - 3))
                    {
                        ...
                        if ((tTemp + 1) < 7)
                        {
                            for (int x = (tTemp + operaValue); x < value; x++)
                            {
                                tByte[x] = (byte)0xff;
                            }
                        }
...
//            操作数值2
            int operaValue = 2;
            for (int i = operaValue; i < value; i++)
            {
                String a = aPan.substring(temp, temp + operaValue);
                tByte[i] = (byte)Integer.parseInt(a, 16);
                temp = temp + operaValue;
            }
        }
        catch (Exception e)
        {
