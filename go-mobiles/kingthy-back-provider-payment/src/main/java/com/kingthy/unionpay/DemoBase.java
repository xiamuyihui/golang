...
public class DemoBase
{
    ...
    {
        int[] lengthArray = {3, 11, 11, 6, 10, 19, 12, 4, 2, 21, 2, 32, 2, 6, 10, 13, 13, 4, 15, 2, 2, 6, 2, 4, 32, 1,
            21, 15, 1, 15, 32, 13, 13, 8, 32, 13, 13, 12, 2, 1, 32, 98};
    ...
    private static List<Map> parseFile(String filePath, int[] lengthArray)
    {
        ...
        try
        {
            ...
            { // 判断文件是否存在
                read = new InputStreamReader(new FileInputStream(file), "iso-8859-1");
                ...
                while ((lineTxt = bufferedReader.readLine()) != null)
                {
                    ...
                    Map<Integer, String> integerStringMap = new LinkedHashMap<Integer, String>();
                    // 左侧游标
                    int leftIndex = 0;
                    // 右侧游标
                    int rightIndex = 0;
                    for (int i = 0; i < lengthArray.length; i++)
                    {
                        ...
                        integerStringMap.put(i, filed);
                    }
                    mapArrayList.add(integerStringMap);
                }
            }
            else
            {
    ...
    public static String getFileContentTable(List<Map> dataList, String file)
    {
        StringBuffer tableSb = new StringBuffer(
            "对账文件的规范参考 https://open.unionpay.com/ajweb/help/file/ 产品接口规范->平台接口规范:文件接口</br> 文件【" + file + "】解析后内容如下：");
